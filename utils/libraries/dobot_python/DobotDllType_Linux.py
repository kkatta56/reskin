...
def load():
    if platform.system() == "Windows":
        ...
        # print("dfsafas",os.path.dirname(__file__)) # Debug line used to add absolute path to the return statement
        return CDLL(os.path.join(os.path.dirname(__file__),"DobotDll.dll"),  RTLD_GLOBAL)
    elif platform.system() == "Darwin":
        return CDLL("./libDobotDll.dylib",  RTLD_GLOBAL)
    elif platform.system() == "Linux":
        try:
            path = __file__
            path = os.path.split(path)[0] + "/libDobotDll.so"
...
def ConnectDobot(api, portName, baudrate):
    ...
    versionBuf = create_string_buffer(100)
    version = "v2.2"
    versionBuf = version.encode("utf-8")
    print(connectInfo.masterDevInfo.type, connectInfo.slaveDevInfo1.type)
    result = api.ConnectDobot(szPara, baudrate, byref(connectInfo), versionBuf)
    if result != DobotConnect.DobotConnect_NoError:
        return [result, 0, 0, 0, 0, 0, 0, 0]
    print('id:', connectInfo.masterDevInfo.devId)
    print('type:', connectInfo.masterDevInfo.type)
    print('fw_name', ''.join(chr(x) for x in connectInfo.masterDevInfo.firmwareName))
    print('fw_ver', [ x for x in connectInfo.masterDevInfo.firwareVersion])
...
def SetHOMECmd(api, temp, isQueued=0):
    ...
    if masterDevType == DevType.Magician:
        ...
        while(True):
            result = api.SetHOMECmd(byref(cmd), isQueued, byref(queuedCmdIndex1))
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
            break
...
def SetPTPCommonParams(api, velocityRatio, accelerationRatio, isQueued=0):
    ...
    pbParam.velocityRatio = c_float(velocityRatio)
    pbParam.accelerationRatio = c_float(accelerationRatio)
    queuedCmdIndex = c_uint64(0)
    
    # 滑轨的特殊处理
    print(slaveDevType, masterDevType)
    print(slaveId, masterId)
    if slaveDevType == DevType.Magician:
        while(True):
